...
- winmm `PlaySound` plays one sound per process: a call replaces the running sound unless it passes `SND_NOSTOP`, in which case the new sound is dropped instead. The per-line tick passes it, the chimes do not. With `SND_MEMORY | SND_ASYNC` the buffer is read after the call returns, so the wave data is pinned for the life of the process (`Sounds`).
