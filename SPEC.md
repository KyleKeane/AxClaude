...
- FR-3.8 SHOULD: Project → **Save conversation as…** (Ctrl+S) writes the conversation as shown (hidden rows left out, wrapped rows joined) as UTF-8 without a byte order mark, default name `<folder>-<yyyyMMdd-HHmm>.txt` in the project folder; announces "Saved".
...
| An answer, in the control area | `Sent: 2, Green` (after Other, `Sent: tulip`), ending `. Next question`, `. Review next` or `. Your words next` when more follows; `Cancelled`; `Saved`; `Question closed` when Claude closed it by itself; `Tick one with Space`; `Keys are at the end`; `Type an answer` |
