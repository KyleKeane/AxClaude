...
| S3 | Answer a prompt | A permission prompt or y/n question appears as plain lines; "Claude needs your answer" is spoken; the user types `1` or `y` and presses Enter. |
...
- FR-2.10: **Nothing typed goes into a question by mistake** (D33). While Claude waits for an answer, Enter sends only an answer: one of the question's keys (`y`, `n`, a number of its list), several keys separated by commas when it takes several, any text while Claude shows an `Enter text for` row (its own words after Other), or, when the question could not be read, `y`, `n` or a number; an answer goes one keystroke per character, then Enter, and an empty field sends Enter as always. Any other text stays in the field and "Claude is waiting for an answer to <title>. Your message was not sent and is still in the field. Press Enter again to send it anyway." is spoken; Enter again with the same text sends it as a message, the way out when a wait is misread. Text sent into a question used to be lost: Claude answers "Please answer y or n." and drops it, and in a list a letter can be a key of the screen.
...
| Input | `TextBox` Multiline AcceptsReturn WordWrap, three lines | "Message to Claude" | AccessibleDescription "Enter sends, Shift+Enter starts a new line". Enter is handled in the field's `KeyDown`. |
...
Input field: Enter send; Shift+Enter new line; Escape guarded; Shift+Tab send Shift+Tab; Ctrl+Up / Ctrl+Down send arrows; plain Up / Down move the caret only; Page Up / Page Down one screen of the field (app, FR-2.1); standard editing keys.
...
- **D33 Claude's questions are answered in a notice (1.4.0; check boxes, own answers and the question record in 1.5.0).** A question at startup (workspace trust) went unheard while the user typed, and the message sent into it was lost: Claude answers text that is not `y` or `n` with "Please answer y or n." and drops it, and in a picker a letter can be a command (`s` in `/model` uses the model for this session only). "Claude needs your answer" alone did not say what was asked. Claude Code gives no machine-readable signal for these questions: hooks carry no question text, fire late, and cover neither the trust dialog nor the pickers. Screen reader mode draws every one in the same shape, though: a title, text lines, numbered or `y.`/`n.` answers (`(selected)` marks the current one), then `Enter y/n:` or `Select with numbers [1-N]. Then Enter to submit or Escape to cancel:` with the cursor on it, and key hints under it. The answer notice (`MainForm.ShowClaudeQuestion`, a `Question` from `AxClaude.Core`) is a plain dialog: the title, a short text with Claude's lines between the title and the answers and "Claude's full question is in the conversation", and the answers as radio buttons in a group named "Claude asks: <title>". The answers are not repeated in the text and Claude's key hints are not shown: a text line that looked like an answer invited Enter on it, which answered with the selected radio button instead. The conversation keeps everything Claude printed, as it always did. The notice opens with its own chime, a quick C6-E6 trill (`Sounds.Notice`, with the other chimes under `soundOnBell`), and the focus starts on the current answer, as in a dialog; that answer's name starts with the notice's title and the question's first line until the focus leaves it (1.5.1: NVDA read only the answer, not the group's name, so the question went unheard). A reply spoken as it arrived went on over a question, and the user did not notice it: replies are not spoken while a question or screen of Claude's shows (1.5.1), and the focus change to the notice cuts off what NVDA was reading. An interrupting "Claude asks" notification raised before the focus was tried and dropped: NVDA took it after the focus and cut off the question instead, and it carries the description "Choose an answer and press Enter, or press Escape to cancel." until the focus leaves it, so the change of screen and the way out are read once with the focus. Closing it says what happened: "Answer sent: 3, Fable", "Question cancelled", or "The question is still open" in the message field. An answer key (a digit, two digits within a second, or `y`/`n`) chooses that answer and moves the focus to it, the arrow keys move between the answers, Enter answers, Escape cancels the question (sends Escape), and Alt+M (*Answer in the message field*) leaves the question open and goes to the message field for typed answers. For the first second after it opens the notice ignores Enter, Space and the answer keys, since it can open while the user is typing. Plain letters never press a button in this notice (only Alt and the letter do). The notice opens by itself: when Claude's cursor sits on a question, the 400 ms attention timer (FR-7.2) reads it off the screen (`SessionModel.PendingQuestion`), `QuestionRouter` chooses the answer notice for a list of answers unless the slash command sent last is a viewer or a panel, and the notice opens in place of the "Claude needs your answer" announcement and its chime (the taskbar still flashes). Enter sends the answer as keystrokes and then Enter (below), Escape sends Escape, and Alt+M leaves the question to the message field (FR-2.10). A question is known by its title and answers (`Question.Signature`): the same one does not open again while Claude waits on it, the next one of a multi-step screen opens its own notice, and a notice whose question went away or changed closes with "The question closed". A question while another notice is open is announced as before. A tool permission prompt in the default mode ("Permission Required: Create file", the file and a preview, "Do you want to create …?", 1. Yes, 2. Yes and accept edits, 3. No) has the same shape and opens the notice (the `permission-prompt` fixture). Options → Answer Claude's questions in a notice (`questionNotices`) turns the notices, the guard and the screen reading off; the `Question:` record and answers typed a keystroke at a time stay, since both are right either way. A question is a prompt row with answers above it; Claude clears a question's rows for a frame while it redraws it, so a prompt row without answers is no question, and a notice closes only once the screen has settled for the attention timer. Several questions in one call (the `several-questions` fixture) come one at a time under a tab row (`←   ☐ Colour   ☐ Fruit   ✔ Submit   →`), which ends the question upwards, as a header row (` ☐ Colour`) does; each is its own question, titled "Question 1 of 2: …" (1.5.1, `Question.Step`: the tab row ticks answered questions ☒ and does not mark the current one, and Claude asks them in order, so the current one is the first ☐), and "Answer sent: …" adds "Next question coming" or "The review of your answers is next", since the moment between two questions sounded like the end; the review at the end (`Review your answers`, a `warning:` row that belongs to it, y/n) is one more. A question whose prompt row says `(comma- or space-separated for several)` takes several answers (`Question.AllowsSeveral`): the notice shows a check box per answer (ticked where Claude marks `(selected)`, described once "Tick answers with Space or their numbers, then press Enter, or press Escape to cancel."), its keys tick and untick, Enter sends the ticked keys joined with commas, and with none ticked it says "Tick at least one answer with Space or its number." and stays; typed in the message field, "1,3" answers too (`Question.Accepts`). Claude's answer field takes typed keys: an answer goes one keystroke per character and then Enter (`SendAnswer`), from the notice and from the field alike, since several characters in one write are a paste, which it ignores ("1 3" and "12" were dropped; 1, comma, 3 as three keystrokes gave "→ Apple, Cherry"). "Other" (the `other-answer` fixture) turns the prompt row into `Enter text for option 4 (Other), or Escape for the list:`, a question for the user's own words (`LineClassifier.IsTextPrompt`, `SessionModel.AwaitsText`; the answers still above it are not read as a question). Other is an answer like any other: its key goes, and the text row that follows (also plan approval's "No, keep planning") opens a follow-up notice (1.5.1, `MainForm.ShowOwnAnswer`, `SessionModel.TextPrompt`), as `/config` does for a setting's value: the question again, "You chose Other.", and a field named with the question, where the focus lands; "Answer sent: 4, Other" adds "Next, type your own answer", so the gap before it does not sound like the end. Enter sends the words one keystroke at a time and Escape goes back to Claude's list, whose notice opens again without the chime. (1.5.0 had the field *Your own answer* under the answers instead; typing the words next to Other was less clear than a step of its own.) In the message field the guard lets any text through on a text row, and it goes one keystroke per character like every answer (the same word in one write was refused with "Enter some text"; 1, comma, 4 and then "kiwi" gave "apple, kiwi"). A permission question starts at its `Permission Required:` row, which is its title, and a `Tip:` row inside it is skipped (the Bash prompt and plan approval, recorded with `--permission-mode default` and `plan`, had the command row or a leftover `/plan to preview` row for a title); its "Tab to amend" draws nothing in screen reader mode and is not offered. "Chat about this" ends the question like Escape: Claude then asks in its reply, and the user answers with a message. Each question or screen the app waits on is recorded once as the line `Question: <title>` in front of its first row (`LineKind.Question`, `SessionModel.MarkQuestion`), found with q and Shift+Q (FR-5): Claude redraws an answered question as a one-line result (`●  User answered Claude's questions:` `· Which colour do you prefer? → Green`, `User rejected write to …`), so the record is what stays of it. **Screen notice.** A screen that waits for keys named on its hint row rather than on a prompt row (`/status`, `/tasks`, `/help`, `/goal`; docs/claude-screens.md) is read by `ScreenReader`: the cursor sits on a hint row naming Escape (`↑/↓ to select · Enter to view · Esc to close`) with the screen's lines above it, or on a tab row (`Settings  Status   Config   Usage   Stats`) with the lines below it down to such a hint row. It sets `SessionModel.PendingScreen` and counts as waiting for an answer (`PromptPending`), so the status, the close question and the guard of FR-2.10 apply. The screen notice shows the title (for a tab screen "Tabs: …") and the lines, which the conversation does not show for a tab screen (its rows are below the cursor, FR-3.5), with a button per key: the action capitalised, with the key's name in brackets unless it is Enter or Escape ("View", "Close", "Select (Down)", "Stop (x)"), and Answer in the message field. A button sends its key alone and closes the notice; a screen is known by its title and keys (`ClaudeScreen.Signature`, not its lines, which can count up by themselves), and after a key of the user's the screen shown next opens again, without the chime when it follows within three seconds. `/mobile` leaves the cursor on the row under its hint row, which is read as the screen's last line. `/config` (1.5.1) lists its settings as a question under its tab row, which ends the question upwards, with the prompt row `Enter a number to change [1-44], or Escape to save and close:` (`Question.IsSettings`). It gets a notice of its own, "Claude's settings" (`MainForm.ShowSettings`): a radio button per setting with its value, Change, and Save and close (Escape). A number flips a true-or-false setting (`QuestionOption.Switch`), so Change on one opens a second notice named after the setting with True and False, and only a changed value sends the number; Change on any other setting sends its number, and Claude's list of values opens as an answer notice. The list comes back with the new value and the focus on the setting just chosen, without the chime (`Notice.StartAnswer`; an answer notice within three seconds of an answer is the user's doing). Nothing about the settings is stored in the app: every list is read off Claude's screen. The keys a list names besides its answers (`s` in `/model`) are not offered: they act on Claude's highlight, which a typed number does not move. `docs/claude-screens.md` lists every slash command with what its screen shows (33 recorded with 2.1.281) and the dialog it gets: none, the answer notice, a screen notice for viewers and panels that wait on a key-hint or tab row, or typed input; `SlashCommands.All` is its code form, and `QuestionReader` and `QuestionRouter` read a question off the screen and choose the dialog.
...
- [ ] Claude's questions and screens (D33, docs/claude-screens.md). Done in 1.5.1: `/config` in the answer notice, `/mobile` in the screen notice, a background agent's bare "↓ to manage" on the mode line counted as background work, `/mcp` and `/hooks` tried; a list's extra keys (`s` in `/model`) are not offered, since they act on Claude's highlight, which typed numbers do not move. Left: the lines of a tab screen (`/status`, `/help`) in the conversation, not only in the notice; `/chrome` and a new theme in `/theme`; the commands still marked not known.
...
`fontFamily` null with `fontSize` 0 is the Windows message font. `claudePath` is set by Locate claude.exe or by hand. `joinWrappedLines` has no menu item. `checkForUpdates` is Options → Check for updates when AxClaude starts (FR-1.10). `questionNotices` is Options → Answer Claude's questions in a notice (D33): off, nothing opens a notice, the guard of FR-2.10 is off, and screens that wait on a hint row are not looked for, so what Claude waits on is only announced, as before 1.4.0; the `Question:` record and answers typed a keystroke at a time stay. `soundOnBell` keeps its 1.0 name and covers the sent, responding, done and question chimes (FR-7.3); `clickOnNewLine` is the tick (FR-7.8). `replySpeech` is `none`, `firstLines` or `all` (FR-7.4); the 1.1 key `speakReplies: true` is read as `all` and no longer written; `speakToolCalls` is Options → Speak each tool call. Written atomically; a missing or unreadable file gives the defaults and a system line. Marker formats and chrome patterns are fixed in code: a pattern change needs a fixture anyway.
