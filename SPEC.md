...
- [ ] Claude's questions and screens, after 1.4.0 (D33, docs/claude-screens.md): buttons for a list's extra keys (`s` and ←/→ in `/model`, Ctrl+A and Ctrl+B in `/resume`; `SlashCommands` has them, `QuestionRoute.ExtraKeys` carries them, the notice does not show them yet); the multi-step screens (a server in `/mcp`, an event in `/hooks`, `/chrome`, a new theme in `/theme`), recorded and tried; the `/config` panel, whose content is not known; the lines of a tab screen (`/status`, `/help`) in the conversation, not only in the notice; `/mobile`, whose cursor sits under its hint row; background subagents on the mode line ("1 agent" is assumed); the commands still marked not known.
