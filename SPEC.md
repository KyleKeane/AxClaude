...
| Control area | `ControlArea` in the message field's place while Claude waits: one `ListView` (Details, one column, no header; `CheckBoxes` when a question takes several) or one single-line `TextBox`, under a `Label` showing the name | the question: "Question 1 of 2: Which colour do you prefer?" | D33. One control at a time, new for every question, so NVDA reads the name each time the focus arrives; `PreviewKeyDown` makes Enter and Escape input keys; from the field's height up to half the window, the list scrolling. |
...
Control area (in the message field's place while Claude waits, D33): Up / Down move; a number or `y` / `n` jumps to its answer; Space ticks; Enter answers or sends the field's words; Escape cancels the question or goes back a step; Tab to the conversation; Shift+Tab send Shift+Tab; Ctrl+Tab / F6 / Ctrl+1 reach it as they reach the field.
...
| Claude asks (control area, D33) | the trill; with the focus at the bottom of the window, nothing more: the focus moves to the list and NVDA reads the question, the role and the answer; with the focus in the conversation, `Claude asks: <question>. Ctrl+Tab to answer` after what NVDA is reading |
| An answer, in the control area | `Answer sent: 2, Green` (after Other, `Answer sent: tulip`), ending `. Next question coming`, `. The review of your answers is next` or `. Next, type your own answer` when more follows; `Question cancelled`; `Settings saved`; `The question closed` when Claude closed it by itself; `Tick at least one answer with Space, then press Enter.`; `The keys are at the end of the list`; `Type your answer first, or press Escape to go back to the list.` |
...
- [ ] Claude's questions and screens (D33, docs/claude-screens.md). Done in 1.5.1: the control area in place of the notices, `/config`, `/mobile`, a background agent's bare "↓ to manage" on the mode line counted as background work, `/mcp` and `/hooks` tried; a list's extra keys (`s` in `/model`) are not offered, since they act on Claude's highlight, which typed numbers do not move. Left: the lines of a tab screen (`/status`, `/help`) in the conversation, not only in the notice; `/chrome` and a new theme in `/theme`; the commands still marked not known.
