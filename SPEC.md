...
- **Options (O):** Announce when Claude is done ✓ | Play sounds: ready, sent, replying, done, question ✓ | Click for each new line from Claude ✓ | Flash the taskbar button when Claude is done ✓ | Speak replies as they arrive ▸ (None ✓ | First lines only | All lines) | Speak each tool call | Show the time in the Input and Output lines | Font… | Larger text Ctrl+Plus | Smaller text Ctrl+Minus | Use the Windows text size | Record raw stream for a bug report… (reads Stop recording while active) | Open settings file
...
Global: Ctrl+Tab / Ctrl+Shift+Tab / F6 toggle field ↔ view; Ctrl+1 field, Ctrl+2 view (announced when already there); Ctrl+Enter send; Ctrl+Shift+S send the waiting message now; Shift+Escape send ESC; Ctrl+F find, F3 / Shift+F3 next / previous; Ctrl+S save; Ctrl+W open the current folder; Ctrl+N new session; Ctrl+Shift+R restart; Ctrl+O send Ctrl+O; Ctrl+Shift+C send Ctrl+C; Ctrl+Shift+M send Shift+Tab; Ctrl+Shift+O latest response; Ctrl+Shift+K bookmark the caret's line; Ctrl+Plus / Ctrl+Minus text size; F1 shortcuts; Alt+F4 exit; Alt menus.
...
- **D31 Sounds are generated, not shipped.** Short sine tones, and noise bursts for the ticks, built in code and played from memory through winmm's `PlaySound`: no media files, no package, and they reach a Remote Desktop client like any other audio. A process plays one sound at a time and a call replaces whatever plays. The per-line tick yields while a chime lasts, timed by the clock from the chime's own length, rather than passing `SND_NOSTOP`: that flag drops the new sound whenever winmm still counts an earlier one as playing, and a sound the audio device never finished would then silence the ticks for good while the chimes went on replacing it (1.1.0 did this; the ticks were reported lost).
