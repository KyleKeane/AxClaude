...
- FR-2.10: **Nothing typed goes into a question by mistake** (D33). While Claude waits for an answer, Ctrl+Enter sends only an answer: one of the question's keys (`y`, `n`, a number of its list), several keys separated by commas when it takes several, any text while Claude shows an `Enter text for` row (its own words after Other), or, when the question could not be read, `y`, `n` or a number; an answer goes one keystroke per character, then Enter, and an empty field sends Enter as always. Any other text stays in the field and "Claude is waiting for an answer to <title>. Your message was not sent and is still in the field. Press Ctrl+Enter again to send it anyway." is spoken; Ctrl+Enter again with the same text sends it as a message, the way out when a wait is misread. Text sent into a question used to be lost: Claude answers "Please answer y or n." and drops it, and in a list a letter can be a key of the screen.
