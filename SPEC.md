...
- FR-8.3: Project → **Change folder…** (menu only: Ctrl+N is New session, FR-8.6, and Ctrl+O is guarded, D14) opens the standard folder picker (`FolderBrowserDialog`, path typeable), then restarts Claude there with the same arguments, the system line `System: Project folder changed to C:\path` and the announcement "Project folder changed to <name>".
