...
1. On the newest version, Help, Check for updates: after a moment the notice "Check for updates" says you have the newest version and names it; Escape closes it. Help, About shows the same version.
...
3. Install an older build: set `<Version>` in `AxClaude.csproj` below the latest release, `.\publish.ps1`, then set it back. Start AxClaude from the Start menu: within a few seconds "AxClaude <newest> is available. See the Help menu", and `s` in the conversation finds the "System:" line saying the same. Options, "Check for updates when AxClaude starts" off, restart the app: nothing is said; turn it on again.
